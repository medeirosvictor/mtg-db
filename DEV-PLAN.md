...
- **Single binary.** Wails produces one executable (~15MB) using the OS native webview. No Electron. No browser dependency.
...
- [ ] Install Wails CLI, Go, Node
- [ ] `wails init -n mtg-db -t svelte-ts`
- [ ] Project structure:
  ```
  app/
    main.go
    ...
    frontend/               ← Svelte app (generated by Wails)
      src/
        App.svelte
        lib/
        views/
  ...
  wishlists/
  history/
  data/                     ← NEW: app-generated data
    cards.db                ← SQLite cache
    images/
      cache/                ← Scryfall images
      custom/               ← User-provided global images
      mpc/                  ← MPC Autofill cached images
  ```
- [ ] Deck parser in Go:
  - Parse all format variants (`1 Name`, `1x Name`, `1x Name (SET) 123`, `*F*`, DFCs)
  - Read `info.md` for commander, colors, status, strategy
  - Read `wishlist.txt`
  - Handle mixed formats within the same file
- [ ] Bind `GetAllDecks()` to frontend — returns list of deck summaries
- [ ] Frontend: render a basic deck list showing name, commander, card count, status
