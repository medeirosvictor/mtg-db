...
<div
  class="bg-bg-secondary border-2 rounded overflow-hidden cursor-pointer hover:border-accent
    {isBasicLand ? 'opacity-70' : ''}
    {isCommander ? 'border-yellow' : ''}
...
>
  <div class="aspect-[488/680] bg-bg-surface overflow-hidden relative">
    ...
    {:else}
      <div class="w-full h-full flex items-center justify-center text-2xl text-text-muted {isNotFound ? 'bg-red/10' : 'bg-gradient-to-br from-bg-surface to-bg-secondary'}">
        {#if isNotFound}
    ...
    {#if card.scryFall?.isDoubleFaced}
      <button
        class="absolute bottom-1.5 right-1.5 bg-black/60 border border-white/20 rounded-full w-7 h-7 flex items-center justify-center cursor-pointer text-sm opacity-0 hover:bg-black/80 hover:border-accent"
  ...
  <div class="p-2 flex flex-col gap-0.5">
    <span 
      class="text-xs text-text-primary whitespace-nowrap overflow-hidden text-ellipsis {isNotFound ? 'text-red' : ''}" 
      title={card.name}
    >
