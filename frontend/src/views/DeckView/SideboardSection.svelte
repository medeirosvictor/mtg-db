...
<section>
  <h2 class="text-base mb-3 text-purple-400">Sideboard ({cards.length} cards)</h2>
  <div class="bg-bg-secondary border-2 border-purple-500/30 rounded overflow-hidden">
        ...
        <span class="w-48 flex-shrink-0 flex gap-1 flex-wrap">
          {#each card.tags || [] as tag}
            <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wide
              {tag === 'commander' ? 'bg-yellow/15 text-yellow' : ''}
              {tag === 'proxy' ? 'bg-yellow/15 text-yellow' : ''}
              {tag === 'sideboard' ? 'bg-purple-500/15 text-purple-400' : ''}
              {tag === 'wishlist' ? 'bg-accent/15 text-accent' : ''}
            ">
              {#if tag === 'commander'}👑{/if}
              {#if tag === 'proxy'}🖨️{/if}
              {#if tag === 'sideboard'}📋{/if}
